...
public class SackTileEntity extends TileEntityInventory implements IFluidHandler {
    ...
    public final ETFluidTank fluidTank = new ETFluidTank(8000);
    ...
    public ItemStack getWrenchDrop(EntityPlayer entityPlayer) {
        return new ItemStack(ElementaryTech.sackBlock, 1);
    }
    ...
    public void updateEntityClient() {
        ...
        if (ElementaryTech.ic2Leaves != null && ElementaryTech.ic2Wood != null) {
            if (this.blobTimer <= 0) {
                if (IC2.platform.isRendering()) {
                    switch (this.currentTree) {
                        case RUBBERTREE:
                            ElementaryTech.proxy.spawnParticle(1, worldObj, xCoord + 0.5D + mX() * 0.5D, yCoord + 1.05D, zCoord + 0.5D + mZ() * 0.5D, -(double) mX() * 0.1D, -0.03D, -(double) mZ() * 0.1D, 0.1F);
                            break;
                        case SPRUCE:
                            ElementaryTech.proxy.spawnParticle(2, worldObj, xCoord + 0.5D + mX() * 0.5D, yCoord + 1.05D, zCoord + 0.5D + mZ() * 0.5D, -(double) mX() * 0.1D, -0.03D, -(double) mZ() * 0.1D, 0.1F);
                            break;
                    }
                }
                if (this.fluidTank.getFluidAmount() >= this.fluidTank.getCapacity()) {
                    if (IC2.platform.isRendering()) {
                        switch (this.currentTree) {
                            case RUBBERTREE:
                                ElementaryTech.proxy.spawnParticle(1, worldObj, xCoord + 0.5D - mX() * 0.3D + (rand.nextDouble() - 0.5D) * mZ() * 0.6D, yCoord + 0.8D, zCoord + 0.5D - mZ() * 0.3D + (rand.nextDouble() - 0.5D) * mX() * 0.6D, 0D, -0.05D, 0D, 0.1F);
                                break;
                            case SPRUCE:
                                ElementaryTech.proxy.spawnParticle(2, worldObj, xCoord + 0.5D - mX() * 0.3D + (rand.nextDouble() - 0.5D) * mZ() * 0.6D, yCoord + 0.8D, zCoord + 0.5D - mZ() * 0.3D + (rand.nextDouble() - 0.5D) * mX() * 0.6D, 0D, -0.05D, 0D, 0.1F);
                                break;
                        }
                    }
                }
        ...
        if (ElementaryTech.ic2Leaves != null && ElementaryTech.ic2Wood != null) {
            if (this.blobTimer <= 0) {
                if (IC2.platform.isSimulating()) {
                    switch (this.currentTree) {
                        case RUBBERTREE:
                            this.fluidTank.fill(new FluidStack(FluidRegistry.getFluid("rubberSap"), blobCapacity), true);
                            break;
                        case SPRUCE:
    ...
    private boolean checkCorrectPlacing() {
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            if (
                meta > 0 &&
                meta2 > 0 &&
                (block == ElementaryTech.ic2Wood || block == ElementaryTech.rubberTreeBlock) &&
                (block2 == ElementaryTech.ic2Wood || block2 == ElementaryTech.rubberTreeBlock)
            ) {
                short newFacing = this.getFacingFromXZ(xz[i], xz[i + 1]);
                this.setFacing(newFacing);
                if (block2 == ElementaryTech.ic2Wood) {
                    worldObj.setBlock(xCoord + xz[i], yCoord + 1, zCoord + xz[i + 1], ElementaryTech.rubberTreeBlock, newFacing, 3);
                } else {
                    if (newFacing != meta2) {
                        return false;
                    }
                ...
                (block == ElementaryTech.spruceTreeBlock && meta > 0)) &&
                ((block2 == Blocks.log && meta2 == 1) ||
                (block2 == ElementaryTech.spruceTreeBlock && meta2 > 0))
            ) {
                short newFacing = this.getFacingFromXZ(xz[i], xz[i + 1]);
                this.setFacing(newFacing);
                if (block2 == Blocks.log) {
                    worldObj.setBlock(xCoord + xz[i], yCoord + 1, zCoord + xz[i + 1], ElementaryTech.spruceTreeBlock, newFacing, 3);
                } else {
                    if (newFacing != meta2) {
                        return false;
                    }
    ...
    private boolean checkGround() {
        ...
        for (int h = 0; h <= this.maxLeavesHeight; h++) {
            ...
            if (!isLogBlock(block, meta)) {
                if (ETUtils.isBlockRegisteredInOreDictionaryAs(block, "blockDirt")) {
                    return true;
                } else {
                    return false;
                }
    ...
    private boolean isLogBlock(Block block, int meta) {
        switch (this.currentTree) {
            case RUBBERTREE:
                return block == ElementaryTech.ic2Wood || block == ElementaryTech.rubberTreeBlock;
            case SPRUCE:
                return (block == Blocks.log && meta == 1) || (block == ElementaryTech.spruceTreeBlock && meta > 0);
        }
        return false;
    }
    ...
    private boolean isLeavesBlock(Block block) {
        switch (this.currentTree) {
            case RUBBERTREE:
                return block == ElementaryTech.ic2Leaves;
            case SPRUCE:
    ...
    private boolean isIncisedLog(Block block) {
        switch (this.currentTree) {
            case RUBBERTREE:
                return block == ElementaryTech.rubberTreeBlock;
            case SPRUCE:
                return block == ElementaryTech.spruceTreeBlock;
        }
        return false;
    }
                ...
                for (int signx = 1; signx >= -1; signx -= 2) {
                    for (int signz = 1; signz >= -1; signz -= 2) {
                        ...
                        if (leavesMatrix[2 + signx * 2][2][iy - 1] == 1 || leavesMatrix[2 + signx][2 + signz][iy - 1] == 1) {
                            ...
                            if (block == ElementaryTech.ic2Leaves) {
