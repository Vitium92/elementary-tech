...
public class UniversalRecipeInput {
		...
		if (iRecipeInputs != null) {
			for (Object material : iRecipeInputs) {
				...
				if (material instanceof ItemStack) {
					...
					String oreDictName = ETUtils.getFirstOreDictNameExcludingTagAny(stack);
	...
	public UniversalRecipeInput(FluidStack fluidInput) {
		this(new Object[] { fluidInput }, null);
	}

	public UniversalRecipeInput(ItemStack itemInput) {
		this(null, new Object[] { itemInput });
	}

	public UniversalRecipeInput(FluidStack[] fluidInputs) {
		this((Object[]) fluidInputs, null);
	}

	public UniversalRecipeInput(FluidStack[] fluidInputs, ItemStack[] itemInputs) {
		this((Object[]) fluidInputs, (Object[]) itemInputs);
	}
	...
	public void sortFluidsByDensity() {
		...
		while (fli.hasNext()) {
			...
			int key = Math.round(ETFluid.getRealDensity(fluid.getFluid()) * 100F);
		...
		if (itemInputs1 != null) {
			...
			while (ii.hasNext()) {
				...
				if (is1 == null || !is.matches(is1)) {
					return false;
				} else if (doCheckAmounts && ETUtils.getAmountOf(is1) < is.getAmount()) {
					return false;
				}
			}
		}
	...
	public boolean containsFluid(FluidStack fluidStack) {
		return containFluidStack(fluidStack);
	}

	public boolean containsItem(ItemStack ingredient) {
		return containItemStack(ingredient);
	}
