...
public class UniversalRecipeOutput {
	...
	public UniversalRecipeOutput(FluidStack[] fluidStacks, Object[] recipeOutputItemStacks, int time1,
			boolean specialConditions1) {
		if (fluidStacks != null) {
	...
	public UniversalRecipeOutput(RecipeOutputItemStack itemOutput, int time1) {
		this.itemOutputs.add(itemOutput);
		this.time = time1;
		this.specialConditions = false;
	}

	public UniversalRecipeOutput(FluidStack fluidOutput, int time1) {
	...
	public boolean matches(List<FluidStack> fluidOutputs1, List<ItemStack> itemOutputs1) {
		if (fluidOutputs.size() != fluidOutputs1.size() || itemOutputs.size() != itemOutputs1.size()) {
			return false;
		}
	...
	public boolean hasFluidOutput() {
		return !fluidOutputs.isEmpty();
	}

	public boolean hasItemOutput() {
		return !itemOutputs.isEmpty();
	}

	public List<ItemStack> getItemOutputStacks() {
		List<ItemStack> result = new ArrayList<ItemStack>();
		for (RecipeOutputItemStack output : itemOutputs) {
			result.add(output.itemStack.copy());
		}
		return result;
	}

	public FluidStack getFirstFluidOutput() {
		return fluidOutputs.isEmpty() ? null : fluidOutputs.get(0).copy();
	}

	public UniversalRecipeOutput copyWithMultiplier(int multiplier) {
		...
		if (itemOutputs != null && !itemOutputs.isEmpty()) {
			...
			for (int i = 0; i < itemOutputs.size(); i++) {
				...
				RecipeOutputItemStack newIs = is.copy(multiplier);
				itemStacks[i] = newIs;
			}
		}
