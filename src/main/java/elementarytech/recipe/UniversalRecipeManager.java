...
public class UniversalRecipeManager {
	...
	public void addRecipe(UniversalRecipeInput input, UniversalRecipeOutput output) {
		...
		while (var8.hasNext()) {
			UniversalRecipeInput existingInput = var8.next();
	...
	public UniversalRecipeOutput getRecipeFor(UniversalRecipeInput input) {
		if (input == null) return null;
		List<FluidStack> fluidInputs = ETUtils.convertRecipeInputToFluidStackList(input.getFluidInputs());
		List<ItemStack> itemInputs = ETUtils.convertRecipeInputToItemStackList(input.getItemInputs());
		return getOutputFor(fluidInputs, itemInputs);
	}
	...
	public UniversalRecipeInput getRecipeInput(UniversalRecipeInput input) {
		...
		List<FluidStack> fluidInputs = ETUtils.convertRecipeInputToFluidStackList(input.getFluidInputs());
		List<ItemStack> itemInputs = ETUtils.convertRecipeInputToItemStackList(input.getItemInputs());
		return getRecipeInput(fluidInputs, itemInputs);
	}
	...
	public void removeRecipeByInput(UniversalRecipeInput uRecipeInput) {
		List<FluidStack> fluidInputs = ETUtils.convertRecipeInputToFluidStackList(uRecipeInput.getFluidInputs());
		List<ItemStack> itemInputs = ETUtils.convertRecipeInputToItemStackList(uRecipeInput.getItemInputs());
		{
			Iterator<Entry<UniversalRecipeInput, UniversalRecipeOutput>> i$ = this.recipes.entrySet().iterator();
			while (i$.hasNext()) {
				Entry<UniversalRecipeInput, UniversalRecipeOutput> entry = i$.next();
