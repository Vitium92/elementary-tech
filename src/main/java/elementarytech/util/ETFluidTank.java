...
public class ETFluidTank implements IFluidTank {
	private final List<FluidStack> fluidList = new ArrayList<FluidStack>();
	private final int capacity;

	public ETFluidTank(int capacity) {
		this.capacity = capacity;
	}

	public ETFluidTank(int capacity, boolean isOpenVessel1) {
		this.capacity = capacity;
	}

	public ETFluidTank readFromNBT(NBTTagCompound nbt) {
		if (!nbt.hasKey("Empty")) {
	...
	public FluidStack getLigthestFluid() {
		if (this.fluidList.isEmpty()) {
			return ETUtils.getFluidStackWithSize("air", this.capacity);
		}
	...
	public FluidStack drain(Object fluidStack, boolean doDrain) {
		...
		} else if (fluidStack instanceof IRecipeInputFluid) {
