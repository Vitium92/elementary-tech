...
public class BronzeTubContainer extends ContainerBase<BronzeTubTileEntity> {
    ...
    public int lastFluidListHash = 0;
    ...
    public void detectAndSendChanges() {
        ...
        int fluidListHash = this.tileEntity.getFluidTank().getFluidList().hashCode();
        for (int i = 0; i < this.crafters.size(); ++i) {
            ICrafting icrafting = (ICrafting) this.crafters.get(i);
            if (fluidListHash != this.lastFluidListHash) {
                IC2.network.get().sendContainerField(this, "fluidList");
            }
            if (this.tileEntity.visibleFluidId != this.lastVisibleFluidId) {
                icrafting.sendProgressBarUpdate(this, 0, this.tileEntity.visibleFluidId);
            }
            if (this.tileEntity.visibleFluidAmount != this.lastVisibleFluidAmount) {
                icrafting.sendProgressBarUpdate(this, 1, this.tileEntity.visibleFluidAmount);
            }
            if (this.tileEntity.progress != this.lastProgress) {
                icrafting.sendProgressBarUpdate(this, 2, this.tileEntity.progress);
            }
        }
        ...
        this.lastFluidListHash = fluidListHash;
    }
