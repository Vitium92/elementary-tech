...
import elementarytech.util.ETInvSlotDischarge;
...
public class ElectricEvaporatorTileEntity extends EvaporatorTileEntity implements IEnergySink, IUpgradableBlock, INetworkTileEntityEventListener {
    ...
    public ElectricEvaporatorTileEntity() {
        ...
        this.fuelSlot = new ETInvSlotDischarge(this, 1, InvSlot.Access.IO, this.tier, InvSlot.InvSide.BOTTOM);
    ...
    public ItemStack getWrenchDrop(EntityPlayer entityPlayer) {
        ...
        for (int i = 0; i < 4; i++) {
            if (this.upgradeSlot.get(i) != null)
                this.worldObj.spawnEntityInWorld(new EntityItem(this.worldObj, this.xCoord, this.yCoord + 1, this.zCoord, this.upgradeSlot.get(i)));
        }
        return new ItemStack(ElementaryTech.electricEvaporatorBlock, 1);
    }
    ...
    public void updateEntityClient() {
        if (((ETInvSlotDischarge) this.fuelSlot).tier != this.tier) {
            ((ETInvSlotDischarge) this.fuelSlot).tier = this.tier;
        }
    }
    ...
    public void updateEntityServer() {
        ...
        if (IC2.platform.isSimulating()) {
            ...
            if (this.getDemandedEnergy() > 1.0D) {
                double amount = ((ETInvSlotDischarge) this.fuelSlot).discharge(this.getDemandedEnergy(), false);
                this.energy += amount;
            }
            if (this.needsFluid()) {
