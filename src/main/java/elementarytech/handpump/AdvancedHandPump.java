package elementarytech.handpump;
...
public class AdvancedHandPump extends HandPump {

    public AdvancedHandPump() {
        super();
        this.maxCharge = ElementaryTech.config.advancedHandpumpMaxCharge;
        this.operationEUCost = ElementaryTech.config.advancedHandpumpOperationEUCost;
        this.tier = ElementaryTech.config.advancedHandpumpTier;
    }

    @Override
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer entityPlayer) {
        MovingObjectPosition movingobjectposition = ETUtils.returnMOPFromPlayer(entityPlayer, world);
        if (movingobjectposition != null) {
            ...
            if (world.isRemote && ElectricItem.manager.use(itemStack, this.operationEUCost, entityPlayer)) {
                entityPlayer.playSound(ModInfo.MODID + ":handpumpOne", (float) (Math.random() * 0.2D + 0.6D), (float) (Math.random() * 0.1D + 0.9D));
                return itemStack;
            } else {
    ...
    private void destroyAllSources(World world, Set<BlockWithCoordinates> blockXYZSet) {
        for (BlockWithCoordinates blockXYZ : blockXYZSet) {
    ...
    private void fillFluidCells(FluidStack fluid1, EntityPlayer player, int countDestroedSources) {
        ItemStack cell = ETFluid.getCell(fluid1.getFluid().getName());
        if (cell == null) {
            return;
        }
        ...
        ETUtils.addItemStackToInventory(player, cell);
        if (emptyCells.stackSize > 0) {
            ETUtils.addItemStackToInventory(player, emptyCells);
        }
    }
    ...
    public void registerIcons(IIconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ModInfo.MODID + ":itemAdvancedElectricHandpump");
    }
